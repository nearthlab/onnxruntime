...
def generate_build_tree(cmake_path, source_dir, build_dir, cuda_home, cudnn_home, tensorrt_home, path_to_protoc_exe, configs, cmake_extra_defines, args, cmake_extra_args):
    ...
    for config in configs:                
        ...
        run_subprocess(cmake_args  + ["-Donnxruntime_DEV_MODE=" + ("ON" if config.lower() == 'debug' and not args.use_tvm and not args.use_ngraph else "OFF"), "-DCMAKE_BUILD_TYPE={}".format(config)], cwd=config_build_dir)
